...
void map(int bk[20][20], int look[4], int xy[2], int move[20][20]) {
	...
	if (kbhit()) {//记录按下的是哪个方向键
		...
		if ((b[0] == 72 || b[0] == 'w') && look[0] != 2)
			//如果输入的为上并且朝向不为下
			look[0] = 1;
		if ((b[0] == 80 || b[0] == 's')&& look[0] != 1)
			look[0] = 2;
		if ((b[0] == 75 || b[0] == 'a')&& look[0] != 4)
			look[0] = 3;
		if ((b[0] == 77 || b[0] == 'd')&& look[0] != 3)
			look[0] = 4;
	}
	switch (look[0]) {
	case 1:
...
int main() {
	...
	case 2: difficulty = 50 ;break;
	case 3: difficulty = 20 ;break;
	}
	restart(bk, look, move);
	while (look[3] == 0) {
		Sleep(difficulty);//休眠400ms一次
		map(bk, look, xy, move);
	}
	gotoxy(35, 8);
	printf("游戏结束，谢谢游玩!^_^");
